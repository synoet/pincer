...
export class Logger implements Logger {
  ...
  async initSession() {
    ...
    if (!userId) {
      ...
      await axios
        .post(`${config.SERVER_URI}/user/create`, {
          userId: userId,
          activated: ACTIVATED,
        })
        .catch(async (err) => {
    ...
    await axios
      .post(`${config.SERVER_URI}/user/session`, {
        userId: userId,
        activated: ACTIVATED,
      ...
      .catch(async (err: any) => {
        await this.error(
          `Failed to Create Session: ${this.sessionId} (Logger-81)`
        );
      });
  }
  ...
  async pingSession() {
    ...
    await axios
      .post(`${config.SERVER_URI}/session/ping`, { sessionId: this.sessionId })
      .catch(async (err) => {
        await this.error(
          `Failed to Ping Session: ${this.sessionId} (Logger-100)`
        );
      });
  }
  ...
  sendSessionLogs(): void {
    ...
    axios
      ...
      .catch(async (err) => {
        await this.error(
          `Failed to post logs for session: ${this.sessionId} (Logger-114))`
        );
      });
  }
  ...
  async pushDocumentLog(log: DocumentLog): Promise<void> {
    await axios
      .post(`${config.SERVER_URI}/document`, {
        sessionId: this.sessionId,
        ...log,
      })
      .catch(async (err: any) => {
        await this.error(
          `Failed to push document log for session: ${this.sessionId} (Logger-135)`
        );
      });
  }
  ...
  async getLastDocumentTime(): Promise<any> {
    const last = await axios
      .get(`${config.SERVER_URI}/document/${this.sessionId}/last`)
      .then((res) => res.data)
      .catch(async (error) => {
        await this.error(
          `Failed to get last document for session: ${this.sessionId} (Logger-142)`
        );
      });
