...
export default async (app: Router) => {
  ...
  /*
    get all users
  */
  app.get("/v2/user", async (req: Request, res: Response): Promise<any> => {
  ...
  app.get("/v2/user/active", async (req: Request, res: Response) => {
    const users = dbClient.collection("users");
    ...
    const activeUsers = await Promise.all(
      allUsers.filter(async (user: any) => {
        if (user.sessions.length === 0) return false;
        await Promise.all(
          user.sessions.filter(async (session: any) => {
            const [currSession] = await sessions
  ...
  */
  ...
  /*
    user statistics
  */
  app.get("/v2/users/stats", async (req: Request, res: Response) => {
    const users = dbClient.collection("users");
    ...
    res.status(200).send({
      ...
      percentActivated: Math.ceil(
        (activatedUsers.length / allUsers.length) * 100
      ),
    });
  });
};
