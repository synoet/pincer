...
export default async (app: Router) => {
  ...
  */
  app.post("/v2/sessions", async (req: Request, res: Response) => {
    ...
    await users.updateOne(
      { userId: userId },
      {
        $set: {
          ...
          sessions:
            user[0].sessions.length > 0
              ? [...user[0].sessions, sessionId]
              : [sessionId],
        },
      },
      (err: any) => {
  ...
  */
  app.get("/v2/sessions", async (req: Request, res: Response) => {
  ...
  */
  app.get("/v2/sessions/:sessionId", async (req: Request, res: Response) => {
    ...
    if (!session)
      res.status(401).send({ message: `No Session with Id ${sessionId}` });
  ...
  */
  app.get("/v2/session/user/:userId", async (req: Request, res: Response) => {
    ...
    await Promise.all(
      userSessionIds.map(async (sessionId: string) => {
        let [session] = await sessions.find({ sessionId: sessionId }).toArray();
  ...
  */
  app.post("/v2/session/ping", async (req: Request, res: Response) => {
  ...
  */
  app.get("/v2/session/stats", async (req: Request, res: Response) => {
    const sessions = await dbClient.collection("sessions");
    ...
    let totalTime = allSessions
      .map((session: any) => {
        let start = new Date(session.startTime),
          end = new Date(session.latestPing);
